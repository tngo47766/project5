...
router.delete('/:photoId/:commentId', jwtAuth, async (req, res) => {
    try {
...
router.put('/update/photo/:photoId/comment/:commentId', jwtAuth, async (req, res) => {
    const { photoId, commentId } = req.params;
    const { comment } = req.body; 
    const userId = req.userId; 
...
router.get('/:photoId/comments', async (req, res) => {
    ...
    try {
        ...
        res.status(200).send({ comments: photo.comments });
    } catch (error) {
        console.error('Server error when fetching comments:', error);
